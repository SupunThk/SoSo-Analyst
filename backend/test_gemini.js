...
const body = {
  ...
  toolConfig: {
    functionCallingConfig: {
      mode: 'ANY',
      allowedFunctionNames: ['get_token_intelligence']
    }
  }
};

axios.post(url, body)
  .then(res => console.log('SUCCESS with ANY'))
  .catch(err => console.error('Error with ANY:', JSON.stringify(err.response?.data || err.message, null, 2)));

body.toolConfig.functionCallingConfig.mode = 'AUTO';
axios.post(url, body)
  .then(res => console.log('SUCCESS with AUTO'))
